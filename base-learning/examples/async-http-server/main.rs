use std::{fs, time::Duration, cmp::min};
...
impl Write for MockTcpStream {
    ...
    fn poll_flush(self: std::pin::Pin<&mut Self>, _: &mut task::Context<'_>) -> task::Poll<std::io::Result<()>> {
        task::Poll::Ready(Ok(()))
    }

    fn poll_close(self: std::pin::Pin<&mut Self>, _: &mut task::Context<'_>) -> task::Poll<std::io::Result<()>> {
        task::Poll::Ready(Ok(()))
    }
}
...
async fn test_handle_connection() {
    ...
    let expected_contents = fs::read_to_string("z_files/base_learning/async-http-server/hello.html").unwrap();
